...
#model_fp32 = SeeInDark()
#model.load_state_dict(torch.load( m_path + m_name ,map_location={'cuda:1':'cuda:0'}))
torch.backends.quantized.engine = 'qnnpack'
model = torch.jit.load('./saved_model/quantized_model.jit', map_location='cpu')
...
for test_id in test_ids:
    #test the all images in each sequence
    in_files = glob.glob(input_dir + '%05d_0*.ARW'%test_id)
